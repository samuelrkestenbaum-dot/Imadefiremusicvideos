#!/usr/bin/env bash
# render_chorus_v16.sh — v14 with a COMPOSITED reflection insert:
...
#
# Usage:  cd when-it-rains && bash scripts/render_chorus_v16.sh && open chorus_v16.mp4
set -euo pipefail
cd "$(dirname "$0")/.."
...
mkdir -p chorus16; : > chorus16/concat.txt
VF="scale=1280:720:force_original_aspect_ratio=decrease,pad=1280:720:(ow-iw)/2:(oh-ih)/2,setsar=1"

get() { [ -s "chorus16/$2" ] || { echo "  get $2"; curl -fSL --retry 4 --retry-delay 2 -o "chorus16/$2" "$1"; }; }
...
seg() { # file in dur idx  — normal trimmed segment
  ffmpeg -nostdin -y -loglevel error -ss "$2" -t "$3" -i "chorus16/$1" -vf "$VF,fps=24,format=yuv420p" -r 24 -an \
    -c:v libx264 -preset medium -crf 18 -video_track_timescale 12800 "$(printf "chorus16/seg_%02d.mp4" "$4")"
  echo "file '$(printf "seg_%02d.mp4" "$4")'" >> chorus16/concat.txt
}
...
ffmpeg -nostdin -y -loglevel error \
  -loop 1 -t 1.95 -i chorus16/empty.png \
  -ss 1.30 -t 1.95 -i chorus16/insert.mp4 \
  ...
  -an -c:v libx264 -preset medium -crf 18 -video_track_timescale 12800 chorus16/seg_04.mp4
echo "file 'seg_04.mp4'" >> chorus16/concat.txt

seg take.mp4   6.90 2.23 5    # beat 2: same take — he turns; no one

( cd chorus16 && ffmpeg -nostdin -y -loglevel error -f concat -safe 0 -i concat.txt -c copy silent.mp4 )
ffmpeg -nostdin -y -loglevel error -i chorus16/silent.mp4 -i audio_relay/chorus_couplet.mp3 \
  -map 0:v:0 -map 1:a:0 -c:v copy -c:a aac -b:a 256k -shortest chorus_v16.mp4
dur=$(ffprobe -v error -show_entries format=duration -of default=nk=1:nw=1 chorus_v16.mp4 2>/dev/null || echo "?")
echo "Wrote chorus_v16.mp4  (${dur}s — composited ghost reflection @ ${GHOST_OPACITY})"
