#!/usr/bin/env bash
# render_chorus_v18.sh — v14 with a COMPOSITED reflection insert:
...
#
# Usage:  cd when-it-rains && bash scripts/render_chorus_v18.sh && open chorus_v18.mp4
set -euo pipefail
cd "$(dirname "$0")/.."
...
mkdir -p chorus18; : > chorus18/concat.txt
VF="scale=1280:720:force_original_aspect_ratio=decrease,pad=1280:720:(ow-iw)/2:(oh-ih)/2,setsar=1"

get() { [ -s "chorus18/$2" ] || { echo "  get $2"; curl -fSL --retry 4 --retry-delay 2 -o "chorus18/$2" "$1"; }; }
...
seg() { # file in dur idx  — normal trimmed segment
  ffmpeg -nostdin -y -loglevel error -ss "$2" -t "$3" -i "chorus18/$1" -vf "$VF,fps=24,format=yuv420p" -r 24 -an \
    -c:v libx264 -preset medium -crf 18 -video_track_timescale 12800 "$(printf "chorus18/seg_%02d.mp4" "$4")"
  echo "file '$(printf "seg_%02d.mp4" "$4")'" >> chorus18/concat.txt
}
...
# seg 4 — the REFLECTION reveal (v18): her face as faint reflected LIGHT on the
...
( cd chorus18 && ffmpeg -nostdin -y -loglevel error -f concat -safe 0 -i concat.txt -c copy silent.mp4 )
ffmpeg -nostdin -y -loglevel error -i chorus18/silent.mp4 -i audio_relay/chorus_couplet.mp3 \
  -map 0:v:0 -map 1:a:0 -c:v copy -c:a aac -b:a 256k -shortest chorus_v18.mp4
dur=$(ffprobe -v error -show_entries format=duration -of default=nk=1:nw=1 chorus_v18.mp4 2>/dev/null || echo "?")
echo "Wrote chorus_v18.mp4  (${dur}s — reflection reveal @ screen ${REFL_OP})"
# QC frames of the reflection reveal (seg4 lands ~7.8-9.8s of chorus_v18)
mkdir -p review/refl18_frames
ffmpeg -nostdin -y -loglevel error -ss 7.6 -t 2.4 -i chorus_v18.mp4 -vf "fps=4,scale=640:-2" -q:v 2 review/refl18_frames/f_%03d.jpg
echo "wrote reflection QC frames"
