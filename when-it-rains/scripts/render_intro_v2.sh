#!/usr/bin/env bash
# render_intro_v2.sh — intro with the LIP-SYNCED first line:
...
GHOST_OPACITY=0.30
mkdir -p intro2; : > intro2/concat.txt
VF="scale=1280:720:force_original_aspect_ratio=decrease,pad=1280:720:(ow-iw)/2:(oh-ih)/2,setsar=1"
get() { [ -s "intro2/$2" ] || { echo "  get $2"; curl -fSL --retry 4 --retry-delay 2 -o "intro2/$2" "$1"; }; }
...
get "$B/hf_20260702_012649_03b68d84-94b0-4751-a6e0-d7847519bd3d.mp4" sync.mp4    # through-glass lip-sync (song 18.5s == clip 0)
get "$B/hf_20260701_234443_f4405992-922d-448f-af9b-4806393a556e.png" shape.png

seg() { ffmpeg -nostdin -y -loglevel error -ss "$2" -t "$3" -i "intro2/$1" -vf "$VF,fps=24,format=yuv420p" -r 24 -an \
  -c:v libx264 -preset medium -crf 18 -video_track_timescale 12800 "$(printf "intro2/seg_%02d.mp4" "$4")"
  echo "file '$(printf "seg_%02d.mp4" "$4")'" >> intro2/concat.txt; }
...
ffmpeg -nostdin -y -loglevel error \
  -ss 2.40 -t 1.00 -i intro2/window.mp4 \
  -loop 1 -t 1.00 -i intro2/shape.png \
  ...
  -an -c:v libx264 -preset medium -crf 18 -video_track_timescale 12800 intro2/seg_06.mp4
echo "file 'seg_06.mp4'" >> intro2/concat.txt

( cd intro2 && ffmpeg -nostdin -y -loglevel error -f concat -safe 0 -i concat.txt -c copy silent.mp4 )
ffmpeg -nostdin -y -loglevel error -i intro2/silent.mp4 -i audio_relay/intro_open24.mp3 \
  -map 0:v:0 -map 1:a:0 -c:v copy -c:a aac -b:a 256k -shortest intro_v2.mp4
dur=$(ffprobe -v error -show_entries format=duration -of default=nk=1:nw=1 intro_v2.mp4 2>/dev/null || echo "?")
echo "Wrote intro_v2.mp4  (${dur}s — first line lip-synced through the glass; maybe-her after the line)"
