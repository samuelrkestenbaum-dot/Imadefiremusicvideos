#!/usr/bin/env bash
# render_chorus_v14.sh — v12 with the GLASSIFIED reflection insert (true glass read):
...
#
# Usage:  cd when-it-rains && bash scripts/render_chorus_v14.sh && open chorus_v14.mp4
set -euo pipefail
cd "$(dirname "$0")/.."
...
get "$B/hf_20260701_222907_6b3be8f1-7d15-401f-b44a-2fbe1fd92289.mp4" insert.mp4  # POV: her single reflection in the case glass
...
for c in "${CUTS[@]}"; do
  set -- $c; f=$1; ip=$2; du=$3
  seg=$(printf "chorus14/seg_%02d.mp4" "$i")
  ffmpeg -nostdin -y -loglevel error -ss "$ip" -t "$du" -i "chorus14/$f" -vf "$VF" -r 24 -an \
    -c:v libx264 -preset medium -crf 18 -video_track_timescale 12800 "$seg"
  echo "file '$(basename "$seg")'" >> chorus14/concat.txt
  i=$((i+1))
done
( cd chorus14 && ffmpeg -nostdin -y -loglevel error -f concat -safe 0 -i concat.txt -c copy silent.mp4 )
ffmpeg -nostdin -y -loglevel error -i chorus14/silent.mp4 -i audio_relay/chorus_couplet.mp3 \
  -map 0:v:0 -map 1:a:0 -c:v copy -c:a aac -b:a 256k -shortest chorus_v14.mp4
dur=$(ffprobe -v error -show_entries format=duration -of default=nk=1:nw=1 chorus_v14.mp4 2>/dev/null || echo "?")
echo "Wrote chorus_v14.mp4  (${dur}s — spec-staged cafe: one take, POV reflection, measured beats)"
