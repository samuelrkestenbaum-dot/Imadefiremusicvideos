#!/usr/bin/env bash
# render_chorus_v12.sh — the cafe scene per CAFE_SCENE_SPEC.md:
...
#
# Usage:  cd when-it-rains && bash scripts/render_chorus_v12.sh && open chorus_v12.mp4
set -euo pipefail
cd "$(dirname "$0")/.."
...
get "$B/hf_20260701_214209_4b32f53e-f94d-4037-95c1-e86e15594e61.mp4" insert.mp4  # POV: her single reflection in the case glass
...
for c in "${CUTS[@]}"; do
  set -- $c; f=$1; ip=$2; du=$3
  seg=$(printf "chorus12/seg_%02d.mp4" "$i")
  ffmpeg -nostdin -y -loglevel error -ss "$ip" -t "$du" -i "chorus12/$f" -vf "$VF" -r 24 -an \
    -c:v libx264 -preset medium -crf 18 -video_track_timescale 12800 "$seg"
  echo "file '$(basename "$seg")'" >> chorus12/concat.txt
  i=$((i+1))
done
( cd chorus12 && ffmpeg -nostdin -y -loglevel error -f concat -safe 0 -i concat.txt -c copy silent.mp4 )
ffmpeg -nostdin -y -loglevel error -i chorus12/silent.mp4 -i audio_relay/chorus_couplet.mp3 \
  -map 0:v:0 -map 1:a:0 -c:v copy -c:a aac -b:a 256k -shortest chorus_v12.mp4
dur=$(ffprobe -v error -show_entries format=duration -of default=nk=1:nw=1 chorus_v12.mp4 2>/dev/null || echo "?")
echo "Wrote chorus_v12.mp4  (${dur}s — spec-staged cafe: one take, POV reflection, measured beats)"
