...
# NOTE: a zoompan push-in on the cold-open window ballooned the clip to 51 min
# (zoompan is a stills/Ken-Burns filter; on video with d=1 it multiplied frames).
# Reverted to the plain (known-good) window. A frame-safe push would use a
# time-based crop (crop=w='iw/(1+0.15*min(t,6)/6)':...), not zoompan.
...
seg rain.mp4    0.50 6.00 0   # 0-6      the first breath (plain, known-good)
